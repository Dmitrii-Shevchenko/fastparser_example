...
object ParserFourthTask extends App {
  import fastparse._
  ...
  def makeParser[_: P](list: List[String]): P[Any] = list match {
    case Nil => End
    case h :: tail =>
      h match {
        case "text"  => text ~ makeParser(tail)
        case "table" => table ~ makeParser(tail)
      }
  }

  def getParse(config: String, file: String) = parse(config, configParse(_)) match {
    case Parsed.Success(configSeq, _) => {
      parse(file, makeParser(configSeq.toList)(_)) match {
